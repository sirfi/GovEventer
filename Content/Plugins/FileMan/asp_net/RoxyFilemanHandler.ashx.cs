...
namespace GovEventer.Content.Plugins.FileMan.asp_net
{
    public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        private string confFile = "../conf.json";
        ...
        public User DataUser
        {
            get
            {
                if (_dataUser == null) _dataUser = _context.Session[AdvancedController.DataUserSessionKey] as User;
                if (_dataUser != null && _context.User.Identity.IsAuthenticated && _dataUser.EMailAddress == _context.User.Identity.Name) return _dataUser;
                if (!_context.User.Identity.IsAuthenticated) return null;
                _dataUser = Db.Users.FirstOrDefault(x => x.EMailAddress == _context.User.Identity.Name);
                _context.Session[AdvancedController.DataUserSessionKey] = _dataUser;
                return _dataUser;
            }
        }
        ...
        public void ProcessRequest(HttpContext context)
        {
            _context = context;
            _r = context.Response;
            if (!context.User.Identity.IsAuthenticated || !DataUser.IsAdmin)
            {
                _r.StatusCode = 403;
                return;
            }
        ...
        protected string GetFilesRoot()
        {
            ...
            //if (ret == "")
            //    ret = _context.Server.MapPath("~/Content/Uploads");
            //else
            //    ret = FixPath(ret);
            //return ret;
            var rootPath = _context.Server.MapPath(Settings.Default.UploadDirPath);
            if (!Directory.Exists(rootPath))
            {
                Directory.CreateDirectory(rootPath);
            }
            var path = Path.Combine(rootPath, DataUser.Id.ToString());
        ...
        protected void VerifyAction(string action)
        {
            ...
            if (setting.IndexOf("?", System.StringComparison.Ordinal) > -1)
                setting = setting.Substring(0, setting.IndexOf("?", System.StringComparison.Ordinal));
