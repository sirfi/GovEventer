(function ($) {
    $.emoticons = $.fn.emoticons = function (options) {
        
        var opts = $.extend({}, $.fn.emoticons.defaults, options);
        var funcs = {};
        $.fn.emoticons.funcs = funcs;
        var emoticons,
            codesMap = {},
            primaryCodesMap = {},
            regexp,
         ...
         */
        funcs.replace = function (text, fn) {
            return text.replace(regexp, function (code) {
                var name = codesMap[code];
                return (fn || funcs.tpl)(name, code, emoticons[name].title);
            });
        };
        ...
        funcs.toString = function (fn) {
            ...
            for (code in primaryCodesMap) {
                if (primaryCodesMap.hasOwnProperty(code)) {
                    ...
                    str += (fn || funcs.tpl)(name, code, emoticons[name].title);
                }
            }
    ...
    $.fn.emoticons.defaults = { type: 'skype', delay: 0, exclude: 'pre,code,.no-emoticons' }
})(jQuery);
